...
class ModifyCartItemHandler implements CommandHandlerInterface
{
    ...
    public function __invoke(ModifyCartItemCommand $command): void
    {
        $cart = $this->getCart($command->cartId());

        $cart->modifyCartItem($command->cartItemId(), $command->quantity());
